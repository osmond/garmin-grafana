...
3. Provide your Garmin credentials through a `.env` file or by exporting the environment variables `GARMINCONNECT_EMAIL` and `GARMINCONNECT_BASE64_PASSWORD` before running docker compose. Create a `.env` file next to `compose.yml` with the following content:
   ```
   GARMINCONNECT_EMAIL=your_email@example.com
   GARMINCONNECT_BASE64_PASSWORD=base64_encoded_password
   ```
   The password value **must** be Base64 encoded. The compose file reads this `.env` file automatically and does not store your credentials in version control. If you set these variables and do not use multi-factor authentication, you can jump directly to `step 5`. Users in mainland China should set `GARMINCONNECT_IS_CN=True`. You can also select what data you want to fetch with the `FETCH_SELECTION` variable in the compose file.
